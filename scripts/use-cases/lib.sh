...
shielded_before() {  # <deploy-tx> <block> <ledger> <shielded-ledger> [<period-blocks>] -> amount
  local f=artifacts/shielded-settlement.tsv pb="${5:-}" payer
  ...
  # row's payer and this then summed EVERY row's amount into it, so a second
  # shielded payer would have had its spends charged to the first one's ledger.
  # There is one payer in the manifest today, which is exactly why the rule was
  # unexercised: "no row belongs to another payer" is true of a file with one
  # payer in it, and stays true right up until it is not.
  payer=$(shielded_payer)
  [ -n "$payer" ] || { echo 0; return 0; }
