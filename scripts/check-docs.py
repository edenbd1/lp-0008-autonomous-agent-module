...
# solutions/LP-0008.md is the document a reviewer is sent to, and was the last
# one anything checked: every path, link and citation in it went unchecked while
# thirteen others were gated.
#
# SUBMISSION-DRAFT.md USED TO BE IN THIS LIST and is deliberately not any more.
# It was 159 KB of working draft at the top level of a public repository, which
# is the opposite of the "clean public repository" the criteria ask for; it is
# now a build input kept outside the tree (see .gitignore). A name left in this
# list would not fail — the loop below skips a document that is not there — and
# that is exactly why it had to be removed rather than left: a list whose
# entries may silently not exist is a list that cannot report a document going
# missing.
...
EXPECTED_OUTSIDE_REPO = {
    "../TERMS.md":
        "solutions/LP-0008.md is submitted into the prize repository, where it "
...
# withdrawal reach the other thirteen documents, and it did not: on 2026-08-19 a
...
RETRACTED = [
    ...
    ("error code 6013 is live",
     ...
     {"docs/limitations.md", "scripts/check-docs.py"}),
    ("the storage skills have never reached a live node",
     r"(?i)storage skills? (?:have|has) never (?:driven|been run|reached)",
     {"docs/limitations.md", "scripts/check-docs.py"}),
    ("there is no video",
     r"(?i)and there is no\s*\n?\s*video\b|no recorded video demo exists",
     {"docs/limitations.md", "scripts/check-docs.py"}),
