...
- four Blockchain skills answer only where a host wires their port, and a loaded
  `.lgx` wires none: `wallet.balance` and `program.query` need a sequencer
  connection, `program.call` and `program.deploy` a `ProgramPort`. Each refuses by
  naming the port it is missing rather than failing vaguely, and the same
  operations *are* on chain through `wallet.send` and the CLI — but inside the
  packaged module those four do not answer. `docs/skills.md` §"Status, honestly"
  carries the per-skill table;
...
- **Reviewed commit:** `13cf8d4`. Every path below is relative to it, and the
...
- [x] **MET — At least 3 of the illustrative use cases above are demonstrated end-to-end on LEZ testnet.**
  Three of the prize's nine run end to end in the recorded demo — marketplace, notary, event alerter. The film also walks the spending threshold, which is not one of the nine but is the criterion the other three rest on, and its narration says so on screen.
...
- [x] **MET — CI must be green on the default branch.**
  Green on `main` and on the reviewed commit specifically, run `33063685508`.
...
- **Video demo:** <https://youtu.be/5HF0xv6GX64> — 21 m 57 s, three of the prize's illustrative
