...
module.** A real Logos Storage node is driven in CI, but by a purpose-built C
driver — that is the library proven, not the skills.
`messaging.create_group` is likewise written against the real API and compiled
and has not been exercised against a running node.
