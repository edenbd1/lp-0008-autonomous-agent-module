...
- [x] **MET — Full documentation — including the skill interface spec, deployment guide, and owner interaction guide — and a clean public repository are delivered.**
  `./scripts/check-docs.py` resolves every path, link and citation across fourteen documents. Public, under MIT / Apache-2.0, and clean at the top level: the 159 KB working draft that used to sit there is a build input kept outside the tree, not a deliverable. [Evidence](../docs/criteria-evidence.md#functionality).
...
- [x] **MET — End-to-end integration tests run against a LEZ sequencer (standalone mode) and are included in CI.**
  [Run `32621320255`](https://github.com/edenbd1/lp-0008-autonomous-agent-module/actions/runs/32621320255) — **success, 3 h 04**, on this branch's head commit `e23cc55` itself, `RISC0_DEV_MODE: 0`, no skip path: it refuses a runner it cannot finish on rather than skipping the proving. [Evidence](../docs/criteria-evidence.md#supportability).
...
across fourteen documents, `check-package-fresh.py` refuses a package whose binary
