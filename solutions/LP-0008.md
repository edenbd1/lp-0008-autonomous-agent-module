...
- [x] **MET — The agent module loads and runs inside Logos Core alongside the
  ...
  **That job is red, and on a later step.** Step 19 drives the file-vault use
  case, and `scripts/use-cases/share_drive.c` does not compile against the
  Delivery library revision this recipe pins — the two want incompatible C ABIs,
  which is neither script's fault and is written up in
  [`docs/limitations.md`](../docs/limitations.md). The three steps above are
  reported as what they are: three green steps inside a red job, not a green job.
...
- [x] **MET — A recorded video demo of the end-to-end flow is included in the
  ...
  **One file, because the requirement is one walkthrough covering three use
  ...
  re-encoding; the two halves stay on the release, labelled as superseded.
