...
- **CI, all green — and this says which run sat where.** The host suite
  (`33084399499`, 9 min) and the explorer-link gate (`33084399822`) ran on the
  reviewed commit itself. The two long jobs did not: the full lifecycle against a
  standalone LEZ sequencer (`32814477623`, 3 h 29, `RISC0_DEV_MODE=0`, no skip
  path) and the run that loads this module in one Logos Core runtime alongside the
  wallet, storage and messaging modules (`32805342163`) both ran on `20678b7`.
  That commit differs from the reviewed one in three markdown files and nothing
  else — no code, no workflow, no script, no artifact — so
  `git diff 20678b7 1956340 --stat` is the whole of the difference, and those two
  runs exercised exactly the code under review. Saying so is cheaper than a
  reviewer discovering it. Beside them run gates that
